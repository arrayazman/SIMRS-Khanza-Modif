...
public final class RMDataResumePasien extends javax.swing.JDialog {
   ...
   private void isRawat() {
         ...
         if(Sequel.cariInteger("select count(no_rawat) from penilaian_medis_igd where no_rawat='"+TNoRw.getText()+"' ")>0){
            ...
            Keluhan.setText(Sequel.cariIsi("select concat(keluhan,'\n',pemeriksaan)as keluhan from pemeriksaan_ralan where no_rawat=? order by jam_rawat desc",TNoRw.getText())); 
            Obat2an.setText(Sequel.cariIsi("select instruksi from pemeriksaan_ralan where no_rawat=? order by jam_rawat desc",TNoRw.getText()));       
        }
    }
